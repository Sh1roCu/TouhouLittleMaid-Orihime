...
public class CuriosContainer extends MaidMainContainer {
    ...
    private final Optional<TrinketComponent> curiosHandler;
    ...
    public CuriosContainer(int id, Inventory inventory, int entityId) {
        ...
        // 延迟添加 Curios 物品栏
        if (maid != null) {
            curiosHandler.ifPresent(this::addCuriosSlotsForPage);
        }
    }
    ...
    public void updatePage(int page, Player player) {
        int curiosSlotsCount = this.curiosHandler.map(cap -> {
            int slots = 0;
            for (var map : cap.getInventory().values()) {
                for (var inv : map.values()) {
                    slots += inv.getSlotType().getAmount();
                }
            }
            return slots;
        }).orElse(0);
    ...
    // FIXME
    // 对于新的女仆实体，打开饰品界面以及放入饰品是正常的（但是Slot的getNoItemIcon获取不到对应SlotType的texture）
    // 退出存档再次进入，会在Container初始化物品时抛出索引异常
    private void addCuriosSlotsForPage(TrinketComponent curios) {
        int total = 0;

        int start = page * SLOTS_PER_PAGE;
        int end = start + SLOTS_PER_PAGE;

        for (var trinkets : curios.getInventory().values()) {
            for (var entry : trinkets.entrySet()) {
                TrinketInventory handler = entry.getValue();
                int maxIndex = handler.getContainerSize();
                var stacks = new MaidCurioSlot.DynamicStackHandler(maxIndex, slot -> new SlotReference(handler, slot));
                ...
                for (int i = 0; i < maxIndex && total < end; i++, total++) {
                    if (total >= start) {
                        ...
                        this.addSlot(new MaidCurioSlot(maid, handler, stacks, i, identifier, x, y, false));
                    }
                }
            }
        }
