package com.github.tartaricacid.touhoulittlemaid.compat.trinkets.client;
...
import com.github.tartaricacid.touhoulittlemaid.compat.trinkets.menu.MaidCurioSlot;
...
public class CuriosContainerScreen extends AbstractMaidContainerGui<CuriosContainer> implements IBackpackContainerScreen {
    ...
    public CuriosContainerScreen(CuriosContainer container, Inventory inv, Component titleIn) {
        ...
        this.maxSlots = TrinketsApi.getTrinketComponent(this.maid).map(cap -> {
            int slots = 0;
            for (var map : cap.getInventory().values()) {
                for (var trinketInv : map.values()) {
                    slots += trinketInv.getSlotType().getAmount();
                }
            }
            return slots;
        }).orElse(0);
    ...
    protected void renderAddition(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.maxPages > 0) {
            ...
            MutableComponent name = Component.literal("Trinkets") /*Component.translatable("curios.name")*/;
